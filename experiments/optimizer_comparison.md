...
| 阶段 | 模型 | n_embd | 参数量 | 训练词元 | 单卡 4090 估时 |
|------|------|--------|--------|---------|--------------|
| **超参校准** | d8 | 512 | ~42M | ~260M | 3-5 min/run |
| **主实验** | d12 | 768 | ~124M | ~900M | 30-40 min/run |
| **消融实验** | d12 | 768 | ~124M | ~900M | 30-40 min/run |
...
- d8 上每次运行 ~500 步（约 3-5 分钟）
- 以 `val_bpb` 收敛曲线为依据选择最优配置
- 预计搜索运行次数：MuonAdamW ~8 次 + AdamW ~10 次 + SOAP ~24 次 ≈ 42 次
- 总搜索时间（单卡 4090）：**~2.5-3.5 小时**

### 4.3 跨尺度超参迁移校验（d8 → d12）

> **风险**：d8（42M 参数）上的最优学习率直接迁移到 d12（124M 参数）可能不准确甚至导致发散。nanochat 对 Muon 的 muP 缩放针对的是 depth 变化时的 LR 自动调整（`dmodel_lr_scale = (model_dim/768)^{-0.5}`），但其他优化器（AdamW、SOAP）未经过此缩放验证。

**迁移校验流程**（在阶段二正式对比之前执行）：

1. 取 d8 上每种优化器的最优 LR 值 `lr*`
2. 在 d12 上以 `lr ∈ {0.5×lr*, lr*, 2×lr*}` 各跑 **100 步**（~3-5 min/run）
3. 观察初期 train_loss 轨迹：是否平稳下降、是否出现爆炸或停滞
4. 选择 d12 上表现最稳定的 LR 作为正式对比的实际值
5. 仅需 ~9 次短运行（3 优化器 × 3 LR），额外耗时 **~30-45 min**
...
### 6.1 阶段一：d8 超参数校准（~2-3 hr）

在 d8 缩微模型上快速搜索各优化器最优超参数。

```bash
# MuonAdamW 校准示例
python -m scripts.base_train --depth=8 --num-iterations=500 --eval-every=100 \
    --optimizer-type=muon --matrix-lr=0.02 \
...
### 6.2 阶段二：d12 正式对比（~2 hr）

使用校准后的最优超参数在 d12 上进行全程训练。

```bash
# 1. MuonAdamW（基线）
python -m scripts.base_train --depth=12 --optimizer-type=muon \
    --matrix-lr=<calibrated> --weight-decay=<calibrated> \
    --run="cmp_muon" --model-tag="cmp_muon"

# 2. 纯 AdamW
python -m scripts.base_train --depth=12 --optimizer-type=adamw \
    --matrix-lr=<calibrated> --weight-decay=<calibrated> \
    --run="cmp_adamw" --model-tag="cmp_adamw"

# 3. SOAP
python -m scripts.base_train --depth=12 --optimizer-type=soap \
...
### 7.1 优化器显存对比（d12，~124M params）

| 优化器 | 参数 | 梯度 | 优化器状态 | 合计（不含激活） |
|--------|------|------|-----------|----------------|
| MuonAdamW | 248 MB | 248 MB | ~250 MB | ~746 MB |
| 纯 AdamW | 248 MB | 248 MB | ~420 MB | ~916 MB |
| SOAP | 248 MB | 248 MB | ~550-700 MB* | ~1046-1196 MB |

*SOAP 额外存储 Kronecker 因子化预条件矩阵（Q_left, Q_right），对 d12 的矩阵形状：
- `(768, 3072)` → Q_left: 768×768 + Q_right: 3072×3072 → ~20 MB/层
- 12 层 × 6 个矩阵 ≈ 额外 ~150-250 MB

### 7.2 GPU 适配

| GPU | VRAM | d8 | d12 | 说明 |
|-----|------|----|-----|------|
| RTX 3060 12GB | 12 GB | ✅ bs≤16 | ✅ bs≤8（SOAP bs≤4） | SOAP 显存较紧张 |
| RTX 4060 Ti 16GB | 16 GB | ✅ bs≤32 | ✅ bs≤16 | 全部优化器舒适运行 |
| RTX 4090 24GB | 24 GB | ✅ | ✅ bs≤32 | 推荐配置 |

### 7.3 总时间预算（单卡 RTX 4090）

| 阶段 | 运行数 | 单次耗时 | 小计 |
|------|-------|---------|------|
| d8 超参搜索 | ~42 | 3-5 min | ~2.5-3.5 hr |
| d8→d12 迁移校验 | 9 | 3-5 min | ~0.5 hr |
| d12 正式对比 | 3 | 30-40 min | ~2 hr |
| d12 消融 | ~7 | 30-40 min | ~4-5 hr |
| **总计** | | | **~9-12 hr** |
